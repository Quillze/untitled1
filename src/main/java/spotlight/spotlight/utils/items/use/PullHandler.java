    ...
    private static void timer(final Player player) {
        (new BukkitRunnable() {
            ...
            public void run() {
                ...
                Collection<Entity> entities = ((World)Objects.requireNonNull(player.getLocation().getWorld())).getNearbyEntities(player.getLocation(), 10.0, 10.0, 10.0, (entityx) -> {
                ...
                PlayerManager.actionBar(player, "&aPulling all monsters for &f" + (10 - this.seconds) + "&a seconds!");
                if (this.ticks >= 20) {
                    ++this.seconds;
                    this.ticks = 0;
                }
